function [min_dist,seg_ind,ctrORend,dist_alongseg,westeast,x_project,y_project] = project_Kpointstofault_westeast(X_line1,Y_line1,X_line2,Y_line2,X_point,Y_point)
...
%     Y_line2 = Sy_end2;
    ...
    for i = 1:length(X_line1) % for each line segment
        ...
        if x1 == x2 && y1 == y2
            ...
            % determine whether point lies east or west of fault
                ...
                elseif slope >= 1 % for steep, positive slopes, positive distance is west
                    find_EW = (dist_P2seg >= 0);
                    if find_EW > 0
                        WestEast(i) = 1;
                    else
            ...
            if within_seg == 1 % if point is normal to a fault segment
                ...
                % order)
                    d1 = sqrt((x1-X_point)^2+(y1-Y_point)^2); % dist to endpoint 1
                    d2 = sqrt((x2-X_point)^2+(y2-Y_point)^2); % dist to endpoint 2
                ...
                if d1 < d2 % if the point is closer to endpoint 1
                    ...
                    Dist_alongseg(i) = 0;
                elseif d2 < d1 % if the point is closet to endpoint 2
                    Min_dist(i) = d2;
    ...
    % if the closest distance to the fault is at a segment junction
    if ctrORend_sort1 == 2 && length(X_line1) > 1
        dn1 = Dist_pt2norm(sort_ind(1)); % distance to normal for first segment 
        dn2 = Dist_pt2norm(sort_ind(2)); % distance to normal for second segment
        if dn1 <= dn2
